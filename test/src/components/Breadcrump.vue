<template>
  <fwb-breadcrumb solid class="bg-gray-200 text-gray-300 rounded-lg p-3 shadow-md">
    ...
    <fwb-breadcrumb-item v-if="currentCategory" href="#" class="max-w-[200px]">
      <router-link 
        :to="`/product-list/${currentCategory}`" 
        class="ml-1 hover:text-gray-100 transition-colors truncate inline-block"
        :title="currentCategory"
      >
        {{ currentCategory }}
      </router-link>
    </fwb-breadcrumb-item>
    <fwb-breadcrumb-item v-if="currentProduct" class="max-w-[200px]">
      <span 
        class="ml-1 font-semibold text-gray-100 truncate inline-block"
