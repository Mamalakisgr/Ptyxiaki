<template>
  <div>
    <!-- Top Header -->
    <header class="bg-gray-800 text-white py-3 px-4 lg:px-6 flex justify-between items-center sticky top-0 z-50 shadow-lg">
      <div class="flex items-center space-x-4 lg:space-x-6">
        ...
        <button 
          ...
          class="relative group focus:outline-none transition-transform duration-200 hover:scale-105 lg:flex"
          data-cy="mobile-menu"
        >
        ...
        <RouterLink to="/" class="flex items-center space-x-3 group">
          ...
          />
          <span class="font-bold text-lg tracking-wide hidden lg:block">
            Fashionistas A.E.
          </span>
        </RouterLink>
      </div>
      ...
      <button 
        ...
        class="lg:hidden p-2 rounded-lg hover:bg-gray-700 transition-colors duration-200"
      >
        <svg xmlns="http://www.w3.org/2000/svg" class="h-6 w-6" fill="none" viewBox="0 0 24 24" stroke="currentColor">
    ...
    <div 
      ...
      class="fixed top-[72px] left-0 right-0 bg-gray-800 p-4 z-50 lg:hidden"
    >
      <div class="relative">
        <input
          type="text"
          ...
          v-model="searchQuery"
          @keyup="searchProducts"
          class="w-full px-4 py-2 rounded-lg bg-gray-700 border border-gray-600 text-white placeholder-gray-400 focus:outline-none focus:ring-2 focus:ring-blue-500 focus:border-transparent"
        />
        <button 
          v-if="searchQuery"
          @click="closeSearchResults"
    ...
    <nav class="lg:hidden fixed bottom-0 left-0 right-0 bg-gray-800 border-t border-gray-700 z-50">
      <div class="grid grid-cols-4 gap-1 p-2">
        <RouterLink 
          to="/" 
          class="flex flex-col items-center justify-center p-2 rounded-lg hover:bg-gray-700"
          :class="{ 'text-blue-500': isCurrentRoute('/')}">
          <svg xmlns="http://www.w3.org/2000/svg" class="h-6 w-6" fill="none" viewBox="0 0 24 24" stroke="currentColor">
        ...
        <RouterLink 
          to="/wishlist" 
          class="flex flex-col items-center justify-center p-2 rounded-lg hover:bg-gray-700 relative"
          :class="{ 'text-blue-500': isCurrentRoute('/wishlist')}">
          <svg xmlns="http://www.w3.org/2000/svg" class="h-6 w-6" fill="none" viewBox="0 0 24 24" stroke="currentColor">
            <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M21 8.25c0-2.485-2.099-4.5-4.688-4.5-1.935 0-3.597 1.126-4.312 2.733-.715-1.607-2.377-2.733-4.313-2.733C5.1 3.75 3 5.765 3 8.25c0 7.22 9 12 9 12s9-4.78 9-12Z" />
          </svg>
          ...
          <span 
            v-if="wishlistCount > 0"
            class="absolute top-1 right-1 w-4 h-4 text-xs flex items-center justify-center bg-red-600 text-white rounded-full"
          >
            {{ wishlistCount }}
          </span>
        </RouterLink>
        ...
        <RouterLink 
          :to="auth.loggedIn ? '/my-account' : '/login'"
          class="flex flex-col items-center justify-center p-2 rounded-lg hover:bg-gray-700"
          :class="{ 'text-blue-500': isCurrentRoute('/my-account') || isCurrentRoute('/login')}">
          <svg xmlns="http://www.w3.org/2000/svg" class="h-6 w-6" fill="none" viewBox="0 0 24 24" stroke="currentColor">
            <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M17.982 18.725A7.488 7.488 0 0 0 12 15.75a7.488 7.488 0 0 0-5.982 2.975m11.963 0a9 9 0 1 0-11.963 0m11.963 0A8.966 8.966 0 0 1 12 21a8.966 8.966 0 0 1-5.982-2.275M15 9.75a3 3 0 1 1-6 0 3 3 0 0 1 6 0Z" />
          </svg>
...
/* Add padding to main content to account for mobile bottom navigation */
